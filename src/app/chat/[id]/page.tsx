...
export default function PaginaChat({
  destinatarioId: propDestinatarioId,
  onClose,
}: {
  ...
  const destinatarioId = propDestinatarioId ?? (params.id as string);
  ...
  // Efeito principal que usa o polling
  useEffect(() => {
    fetchAndSyncMessages();
    const intervalId = setInterval(fetchAndSyncMessages, 2000); // Poll a cada 2 segundos
  ...
  const enviarMensagem = async (
    ...
    setTexto("");
    setResposta(null);
    fetchAndSyncMessages(); // Garante que a conversa atualiza na hora
  };
